...
public class MainActivity extends Activity {
    ...
    private OnClickListener actionPerformed = new OnClickListener() {
        ...
        public void onClick(View v) {
            ...
            // 若输入正确，则将输入信息显示到显示器上
            if (("0123456789.()sincos" +
                    "tanlnlogn!+-×÷√^").indexOf(command) != -1
                    && tip_lock) {
                // 共25个按键
                print(command);
                ...
                if (tip_i > 0)
                    tip_i--;
                // 如果是在按=之后输入退格键
            } else if (command.compareTo("退格") == 0 && equals_flag == false) {
                // 将显示器内容设置为0
                input.setText("0");
                vbegin = true;
                tip_i = 0;
                ...
                tip.setText("欢迎使用！");
                // 如果输入的是归零键
            } else if (command.compareTo("归零") == 0) {
                // 将显示器内容设置为0
                input.setText("0");
                // 重新输入标志置为true
                vbegin = true;
                ...
                tip.setText("欢迎使用！");
                // 如果输入的是”清除“，则将存储器内容清0
            } else if (command.compareTo("清除") == 0) {
                mem.setText("0");
                // 如果按”退出“则退出程序
            } else if (command.compareTo("退出") == 0) {
                System.exit(0);
                // 如果输入的是=号，并且输入合法
    ...
    private void TipChecker(String tipcommand1, String tipcommand2) {
        ...
        // 若命令式以下之一，则显示相应的帮助信息
        if (tipcommand2.compareTo("清除") == 0)
            Tipcode2 = 1;
        if (tipcommand2.compareTo("归零") == 0)
            Tipcode2 = 2;
        if (tipcommand2.compareTo("DRG") == 0)
            Tipcode2 = 3;
        if (tipcommand2.compareTo("退格") == 0)
            Tipcode2 = 4;
    ...
    /*
     * 检测函数，返回值为3、2、1 表示应当一次删除几个， Three+Two+One = TTO 为退格按钮的删除方式提供依据
     */
    ...
    public class calc {
        ...
        public void process(String str) {
            ...
            tip.setText("计算完毕，要继续请按归零键 ");
            mem.setText(str_old + "=" + String.valueOf(FP(number[0])));
        }

        /*
         * FP = floating point 控制小数位数
         */
